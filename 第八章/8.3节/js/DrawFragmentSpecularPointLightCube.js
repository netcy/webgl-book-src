...
var fragmentSource=  ' precision mediump float;\n '
 ...
 + '     float rdotv = max(dot(reflectionDirection, eyeDirection), 0.0);' 
 + '     float specularLightWeight =  pow(rdotv, uShininess);\n'                                              
 + '     vec3 specular = uLightColor * specularLightWeight * vec3(vColor); \n'
 + '     vec4 color = vec4(ambient + diffuse * normalDotDirection  + specular,vColor.a);\n'
 + '     gl_FragColor = color;\n' // 设置颜色
 + '}';
...
function load(){
  ...
  var program = utils.buildProgram(gl,vertexSource,fragmentSource); // 创建着色器程序 
     ...
     var uCameraPosition = utils.getVariableLocation(gl,program,'uCameraPosition',false);
     var uShininess = utils.getVariableLocation(gl,program,'uShininess',false);

     var verticesColorsNormals = new Float32Array([
           // 前面四个顶点
             -10,  10,  10, 1,1,1, 0,0,1,//v0  红色 法线向量 0,0,1
            -10, -10,  10, 1,1,1,0,0,1,// v1 红色
              10,  10,  10,  1,1,1,0,0,1,// v2 红色
              10,  -10, 10,  1,1,1,0,0,1,// v3 红色
           // 右面四个顶点
     ...
     var indices = new Uint8Array([
      ...
      var lightColor = [.7,0.7,0.7],lightPosition = [0,0,15],ambientLightColor = [0.3,0.3,0.3];
      gl.enable(gl.DEPTH_TEST); 
      function draw(){
            var viewMatrix = mat4.create();
            cameraPosition = [radius * Math.sin(angle),0,radius * Math.cos(angle)];
            mat4.lookAt(viewMatrix,cameraPosition,[0.0,0,0],[0,1,0]);
