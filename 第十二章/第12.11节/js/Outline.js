...
function load(){
     ...
     var uOutline = utils.getVariableLocation(gl,program,'uOutline',false);
      ...
      function drawCube(){
            enableStencilWrite();
            var modelMatrix = mat4.create(),viewMatrix = mat4.create(),projectMatrix = mat4.create();
      ...
      function drawOutline(){
          enableStencilTest();
           var modelMatrix = mat4.create(),viewMatrix = mat4.create(),projectMatrix = mat4.create();
           mat4.fromScaling(modelMatrix,[1.02,1.02,1.02]);
            mat4.lookAt(viewMatrix,[radius * Math.sin(angle),20,radius * Math.cos(angle)],[0.0,0,0],[0,1,0]);
            mat4.perspective(projectMatrix,Math.PI/4,1,0.1,1000);
            gl.uniformMatrix4fv(uModelMatrix,false,modelMatrix);
            gl.uniformMatrix4fv(uViewMatrix,false,viewMatrix);
            gl.uniformMatrix4fv(uProjectMatrix,false,projectMatrix);
            gl.uniform1i(uOutline,true);
            gl.drawElements(gl.TRIANGLES,indices.length,gl.UNSIGNED_BYTE,0);
      }
